...
(define (scm-tx->base64 scm-tx)
  (run-subprocess-pipeline/out
   `(,(λ () (write scm-tx))) ; write as datum that can be read back by a guile script
   `(,@docker-run-prefix serialize.sh TransactionEnvelope)))
...
(module+ test
  ...
  (define/provide-test-suite sign/test
    (test-case
     ...
     (check-equal?
      (sign (scm-tx->base64 scm-tx) priv) signed-tx))))
